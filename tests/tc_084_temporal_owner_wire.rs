...
/// `agent-ix-baseline-producer` dependency's own public types.
///
/// This is the minimal document `StaticProducerBundle::admit` accepts: one
/// header (identity, revision, digest, interface version), a selected model
/// and profile, an empty-but-closed inventory, and a static closure carrying
/// one declaration source. Nothing here is a fixture — every value is
/// authored inline and the bundle's own digest, and its configuration
/// document's digest, are computed with the crate's own canonicalizer
/// (`configuration_digest`, `StaticProducerBundle::canonical_digest_selection`)
/// rather than copied from anywhere.
fn minimal_producer_bundle() -> AdmittedStaticBundle {
    let dummy_digest = format!("sha256:{}", "0".repeat(64));

    let mut configuration = ConfigurationDocument {
        configuration_identity: "configuration:tc-084".to_owned(),
        baseline_version: BASELINE_VERSION.to_owned(),
        digest: DigestSelection::canonical(dummy_digest.clone()),
        model_authority: "authority:tc-084".to_owned(),
        profile_identities: BTreeSet::new(),
        adapter_identities: BTreeSet::new(),
        mapping_targets: BTreeSet::new(),
        loss_policy: "loss:tc-084".to_owned(),
    ...
    let static_closure = StaticClosure {
        ...
        declaration_sources: vec![DeclarationSource {
            source: ArtifactReference {
                ...
                authority: "authority:tc-084".to_owned(),
                identity: "source:tc-084".to_owned(),
                revision: ProducerRevision::producer("1"),
                digest: RawByteDigest::new(dummy_digest.clone()).unwrap(),
                wire: WireReference {
                    identity: "wire:tc-084".to_owned(),
                    version: "1".to_owned(),
                },
            },
            native: NativeSourceLabel::new("native:tc-084", "label-1"),
            path: "tc-084.md".to_owned(),
            formal: FormalDocument {
                document: "formal:tc-084".to_owned(),
                revision: ProducerRevision::producer("1"),
            },
        }],
    };
    ...
    let mut bundle = StaticProducerBundle {
        bundle_identity: Some("bundle:tc-084".to_owned()),
    ...
    bundle
        .admit()
        .expect("the minimal TC-084 producer bundle is admitted")
}
...
fn qualified_with_clock(
    ...
    let producer = minimal_producer_bundle();
