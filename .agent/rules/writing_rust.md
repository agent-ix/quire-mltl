...
- `make lint` runs `cargo clippy --all-targets --all-features -- -D warnings`. Warnings are errors.
- 100-char max width (rustfmt).
...
- `make deny` enforces the allowlisted licenses in `deny.toml` (MIT, Apache-2.0, Unicode-3.0,
  AGPL-3.0-only, AGPL-3.0-or-later — this crate's own dependency graph is AGPL, mirroring
  `tl-mltl`'s allow list).
- New crates with other licenses require an explicit `deny.toml` exception with a comment explaining why.
- Unknown registries and unknown git sources are denied. `[sources].allow-git` names the
  exact `tl-mltl`, `quire-observation`, `tl-syntax`, and `filament-core-data` git sources
  this crate pins.
