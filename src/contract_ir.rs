...
//!
//! Ported from `tl-mltl`'s `mapping::contract_ir` (TL-178/TL-175). This
//! module deliberately owns no Contract-IR vocabulary, parser, evaluator,
//! Boolean coercion, callback, or trust flag. It derives a closed TL-owned
//! map only from a constructor-private [`ValidatedTemporalResult`]. See
//! FR-003.
...
use tl_mltl::wire::{OwnerLimits, OwnerReadError, OwnerReadErrorCode, OwnerUsage};

use crate::report::{AssessmentExecution, ResultWire, TemporalTruth, ValidatedTemporalResult};
...
/// Value-or-non-value projection owned by quire-mltl.
#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
